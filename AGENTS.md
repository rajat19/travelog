...
├── skills/                        # AI skill files (see §6)
│   ├── add_country.md             # Step-by-step guide: adding a new country
│   └── add_city.md                # Step-by-step guide: adding a new city
├── public/
...
Detailed, step-by-step AI skill files live in the `skills/` directory at the
project root. **Read the relevant skill file before making changes:**

| Task               | Skill File                  |
| ------------------ | --------------------------- |
| Add a new country  | `skills/add_country.md`     |
| Add a new city     | `skills/add_city.md`        |

Each skill covers: data entry in `travel.ts`, MDX content creation (with full
frontmatter and custom component templates), image generation / placement,
coordinate handling (including low-res GeoJSON nudging), and a verification
checklist.

### Quick Summary

1. **Data** — Add a `Country` / `City` object to `src/data/travel.ts`
2. **Content** — Create MDX files under `src/content/countries/<slug>/`
3. **Images** — Place cover + gallery images in `public/images/<slug>/`
